...
func RemoveFinishedJobs(orders []Order) {
...
func MarkOrderAsDone(orders []Order, match Order) {
...
func AddClient(clients map[network.ID]Client,
               sender_id network.ID,
               client_timed_out chan network.ID) Client {
...
func MasterLoop(c Channels, backup network.ID, initial_queue []Order) {
    ...
    for {
        ...
        case packet := <- c.from_client:
            ...
            if !exists {
                client = AddClient(clients, sender_id, client_timed_out)
            }

            client.Timer.Reset(TIMEOUT_INTERVAL)
            client.LastPassedFloor = packet.LastPassedFloor
            clients[sender_id] = client

            for _, r := range(packet.Requests) {
                if IsNewOrder(r, orders) {
                    if r.Button.Type == driver.ButtonOut {
                        r.TakenBy = sender_id
                    }
                    orders = append(orders, r)
                } else if r.Done {
                    MarkOrderAsDone(orders, r)
                }
            }

            RemoveFinishedJobs(orders)

        case <- time_to_send.C:
            DistributeWork(clients, orders)
            c.to_clients <- MasterData {
