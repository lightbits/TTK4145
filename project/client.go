...
func WaitForMaster(
    ...
    net_events       network.ClientEvents,
    remaining_orders []queue.Order) {
    ...
    for {
        select {
        case packet := <- net_events.from_master:
            ClientLoop(c, packet.Address)
            return

        case <- time_to_ping.C:
            net_events.to_master <- network.ClientData{}

        case button := <- io_events.button_pressed:
            if button.Type == driver.ButtonOut {
                // TODO: Add to order list
            }

        case <- io_events.floor_reached:
        case <- io_events.stop_button: // ignore
        case <- io_events.obstruction: // ignore
        }
...
func ClientLoop(
    ...
    for {
        ...
        case <- time_to_send.C:
            c.to_master <- network.ClientData {
        ...
        case floor := <- lift_events.last_passed_floor_changed:
            last_passed_floor = floor

        case floor := <- lift_events.completed_floor:
        ...
        case data := <- net_events.from_master:
